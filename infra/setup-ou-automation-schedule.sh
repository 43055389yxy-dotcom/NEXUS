...
RULE_ARN="$(aws events put-rule --name "$RULE_NAME" --schedule-expression 'cron(0 18 * * ? *)' --state ENABLED --description 'NEXUS daily customer guardrail sync at 02:00 Asia/Shanghai' --region "$REGION" --query RuleArn --output text)"
...
echo "已配置：每天北京时间 02:00 同步客户账号限制"
