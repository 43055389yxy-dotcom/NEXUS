...
RULE_ARN="$(aws events put-rule --name "$RULE_NAME" --schedule-expression 'cron(0 4 * * ? *)' --state ENABLED --description 'NEXUS Support billing check at 12:00 Asia/Shanghai; application runs every 48 hours' --region "$REGION" --query RuleArn --output text)"
