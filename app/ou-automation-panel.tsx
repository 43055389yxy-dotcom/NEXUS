...
type MoveDestination = 'ungrouped' | 'restricted' | 'temporary';
  ...
  return <>
    ...
    {open && <div className={styles.layer} onMouseDown={(event) => event.target === event.currentTarget && setOpen(false)}>
      <section className={styles.dialog} role="dialog" aria-modal="true">
        ...
        <div className={styles.toolbar}><strong>代付账号 {accounts.length}</strong><input value={accountQuery} onChange={(event) => setAccountQuery(event.target.value)} placeholder="搜索代付账号" aria-label="搜索代付账号" /><button disabled={busy || accounts.length === 0 || previewMode} onClick={() => void run()}>{busy ? '处理中' : '全部归位'}</button></div>
        <div className={styles.layout}>
          ...
          <section className={styles.config}>{busy && !discovery ? <div className={styles.empty}>正在读取...</div> : !discovery ? <div className={styles.empty}>选择一个代付账号</div> : <>
            ...
            <div className={styles.tabs}>{(['all', 'ungrouped', 'restricted', 'temporary'] as const).map((value) => <button key={value} className={memberFilter === value ? styles.selectedTab : ''} onClick={() => setMemberFilter(value)}>{placementLabel(value)}</button>)}</div>
            <div className={styles.memberList}>{visibleMembers.length === 0 ? <p>没有匹配的成员账号</p> : visibleMembers.map((member) => <div className={styles.memberRow} key={member.accountId}><div><strong>{member.name}</strong><small>{member.email}</small></div><code>{member.accountId}</code><span data-placement={member.placement}>{member.parentName}</span><select value={member.placement} disabled={busy || previewMode || !discovery.temporaryOuId || !discovery.restrictedOuId} onChange={(event) => setPendingMove({ member, destination: event.target.value as MoveDestination })}>{member.placement === 'other' && <option value="other" disabled>{member.parentName}</option>}<option value="restricted">禁止 SP/RI</option><option value="temporary">临时</option><option value="ungrouped">未分组</option></select></div>)}</div>
          </>}</section>
        </div>
