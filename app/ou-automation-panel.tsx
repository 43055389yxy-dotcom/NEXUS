...
type OuOption = { id: string; name: string; match?: 'created' };
...
export const OuAutomationPanel = forwardRef<OuAutomationHandle, { onNotice: (message: string) => void }>(function OuAutomationPanel({ onNotice }, ref) {
  ...
  async function beginInitialization(accountId: string) {
    const payload = await request({ action: 'initialize', accountId }) as { result?: { message?: string }; discovery?: Discovery; preview?: boolean };
    ...
    onNotice(payload.result?.message ?? 'OU 初始化并归位完成');
