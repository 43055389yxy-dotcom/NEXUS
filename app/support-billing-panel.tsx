...
export function SupportBillingPanel({ onNotice }: { onNotice: (message: string) => void }) {
  ...
  async function setAccountAutoSync(account: BillingRow, enabled: boolean) {
    const aggregateView = selectedPayerRef.current === ALL_PAYERS_ID;
    const payerId = aggregateView ? account.payerId : selectedPayerRef.current;
    ...
    try {
      ...
      const historyMonth = historicalMonthFor(periodRef.current);
      const visibleSnapshot = aggregateView ? allSnapshots[payerId] ?? null : snapshot;
      const nextSnapshot = historyMonth ? (visibleSnapshot ? withAutoSync(visibleSnapshot, account.id, enabled) : null) : payload.snapshot ?? (visibleSnapshot ? withAutoSync(visibleSnapshot, account.id, enabled) : null);
      if (nextSnapshot) {
        cacheRef.current?.put(payerId, { snapshot: nextSnapshot, preview }, historyMonth);
        if (aggregateView) setAllSnapshots((current) => ({ ...current, [payerId]: nextSnapshot }));
      ...
      <section className={styles.dialog} role="dialog" aria-modal="true">
        ...
        <div className={styles.layout}>
          ...
          <main className={styles.content}>
            ...
            <div className={styles.stats}><button className={rowFilter === 'pending' ? styles.statActive : ''} onClick={() => { setRowFilter((current) => current === 'pending' ? 'all' : 'pending'); setSelected([]); }}><b>{summary.pending}</b>待处理</button><button className={rowFilter === 'synced' ? styles.statActive : ''} onClick={() => { setRowFilter((current) => current === 'synced' ? 'all' : 'synced'); setSelected([]); }}><b>{summary.synced}</b>已同步</button><button className={rowFilter === 'all' ? styles.statActive : ''} onClick={() => { setRowFilter('all'); setSelected([]); }}><b>{sourceRows.length}</b>全部账号</button><button className={rowFilter === 'blocked' ? styles.statActive : ''} onClick={() => { setRowFilter((current) => current === 'blocked' ? 'all' : 'blocked'); setSelected([]); }}><b>{summary.blocked}</b>已拦截</button><button className={rowFilter === 'enabled' ? styles.statActive : ''} onClick={() => { setRowFilter((current) => current === 'enabled' ? 'all' : 'enabled'); setSelected([]); }}><b>{summary.enabled}</b>自动同步开启</button><input value={memberQuery} onChange={(event) => setMemberQuery(event.target.value)} placeholder="搜索名称或账号 ID" /></div>
            ...
            <div className={styles.tableWrap}><table><thead><tr><th>选择</th><th>成员账号</th><th>账期</th><th>账单组 / 同步资格</th><th>自动同步</th><th>最近检查</th><th>AWS Support</th><th>当前同步</th><th>差额</th><th>状态</th></tr></thead><tbody>{busy && sourceRows.length === 0 ? <tr><td colSpan={10}>正在读取...</td></tr> : rows.length === 0 ? <tr><td colSpan={10}>暂无扫描结果</td></tr> : rows.map((account) => { const item = periodItem(account); const selectable = !readOnlyPeriod && (safeToSync(item.status) || canDelete(account, recentPeriod, billingMonth)); const difference = item.aws === null ? null : item.aws - (item.synced ?? 0); const autoSync = accountAutoSyncEnabled(account, account.payerInfo); const lastWrite = account.history?.find((entry) => entry.action !== '删除'); const visibleSnapshot = isAllPayers ? allSnapshots[account.payerId] : snapshot; const lastCheckedAt = visibleSnapshot?.lastScanAt ?? account.payerInfo?.lastScanAt; return <tr key={`${account.payerId}:${account.id}`} data-risk={risky(item.status)}><td><input type="checkbox" disabled={busy || !selectable} checked={selected.includes(account.id)} onChange={(event) => setSelected((current) => event.target.checked ? [...current, account.id] : current.filter((value) => value !== account.id))} /></td><td><strong>{account.name}</strong><small>{account.id}{isAllPayers ? ` · ${account.payerRemark}` : ''}</small></td><td className={styles.billingMonth}>{billingMonthLabel}</td><td>{billingGroupLabel(account.payerInfo?.architecture, account.cma, item, readOnlyPeriod)}</td><td><button type="button" aria-pressed={autoSync} title={autoSync ? '每天检查时允许自动同步；人工同步始终可用' : '自动写入已关闭；扫描展示和人工同步不受影响'} className={`${styles.autoSyncToggle} ${autoSync ? styles.autoSyncOn : styles.autoSyncOff}`} disabled={busy} onClick={() => void setAccountAutoSync(account, !autoSync)}>{autoSync ? '已开启' : '已关闭'}</button></td><td className={styles.lastSync}>{lastCheckedAt ? <><time dateTime={lastCheckedAt}>{formatTime(lastCheckedAt)}</time><small>{lastWrite ? `最近写入 ${formatTime(lastWrite.date)} · ${lastWrite.action}` : '检查完成，无需写入'}</small></> : '暂无检查记录'}</td><td>{money(item.aws)}</td><td>{money(item.synced)}</td><td>{difference === null ? '—' : `${difference > 0 ? '+' : ''}${money(difference)}`}</td><td><span data-status={item.status} title={item.suggestion}>{statusText[item.status] ?? item.suggestion}</span></td></tr>; })}</tbody></table></div>
          </main>
        </div>
